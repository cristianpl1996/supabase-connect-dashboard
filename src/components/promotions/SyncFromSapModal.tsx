...
export default function SyncFromSapModal({ open, onClose, onImported, onBusyChange }: SyncFromSapModalProps) {
  ...
  // Reset to idle when closed after seeing a final state
  useEffect(() => {
    if (!open && state !== 'importing') {
      if (previewPollRef.current) { clearInterval(previewPollRef.current); previewPollRef.current = null; }
      if (state === 'result' || state === 'error' || state === 'preview' || state === 'loading') {
        setState('idle');
      }
    }
    if (!open && state === 'importing') {
      // import continues in background — clean up if user force-closes
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Poll /auto-status until the background import completes
