...
import { Settings as SettingsIcon, ShieldCheck, Users, RotateCcw, Plus, Trash2, Info } from 'lucide-react';
...
const Settings = () => {
  const { config, concepts, toggleRule, resetToDefaults } = useBudgetRules();
  ...
  const handleResetRules = () => {
    resetToDefaults();
    toast.success('Reglas restauradas a valores por defecto');
  };
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {spendableCount} de {concepts.length} conceptos marcados como gastables
                    </CardDescription>
                  </div>
                  <Button
                    variant="outline"
              ...
              <CardContent>
                <div className="divide-y divide-border">
                  {concepts.map((concept) => {
                    const isActive = config[concept.key] ?? concept.defaultSpendable;
                    return (
                      <div
                        key={concept.key}
                      ...
                      >
                        ...
                        <Switch
                          id={concept.key}
                          checked={isActive}
                          onCheckedChange={() => toggleRule(concept.key)}
                          aria-label={`¿${concept.label} es presupuesto gastable?`}
                        />
                      </div>
                    );
                  })}
